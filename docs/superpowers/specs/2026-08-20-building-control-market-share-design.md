...
Before historical backfill, a live API contract spike is a release gate. It records fixtures and verifies the supported bulk search operations, date parameters and inclusivity, exact server-side product filters when available, `totalCount`/pagination behavior, parent and detailed product-code fields, final-result status values, final award date, revision identity, representative-winner business number/name, and API-key permission. If an exact server-side product filter is unavailable, the persisted/resumable fallback paginates the relevant goods records and applies the exact purchase-target check locally. It respects daily limits and never marks unvisited pages complete.

Representative attribution uses only an official representative marker and business identity such as `bidwinnrBizno` confirmed by the contract spike. A single unambiguous winner is acceptable. Multiple rows must resolve to one consistent officially marked representative; the implementation must not pick the first row or infer from company name. Missing, conflicting, or multiple unmarked identities are quarantined and keep that award-date coverage incomplete rather than assigning the notice incorrectly.
...
- `selectElpdtSlctnSttusDtl.do` is the authoritative source for deciding whether a designation contains classification `39121801`.
